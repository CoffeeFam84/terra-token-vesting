...
use cosmwasm_std:: {
  from_binary, DepsMut, Env, MessageInfo, Response, StdResult, StdError
};
...
pub fn instantiate (
  ...
  CONFIGURATION.save(
    deps.storage,
    &Config {
      vesting_token: deps.api.addr_canonicalize(&msg.vesting_token)?,
    ...
    Cw20HookMsg::AddSchedule {
      ...
      if let token_addr = config.vesting_token {
        if deps.api.addr_humanize(&token_addr)? == info.sender.clone() {
          return add_schedule(deps, env, cw20_msg.amount, beneficiary, lock_period, release_period)
        }
      }
      Err(StdError::genric_err("Invalid Cw20 Token"))
    }
  }
}
...
pub fn add_schedule(
  ...
  if amount.is_zero() {
    return Err(StdError::genric_err("amount is zero"));
  }
  ...
  let mut schedule = SCHEDULES
    ...
    schedule.beneficiary = deps.api.addr_canonicalize(&beneficiary)?;
    ...
    SCHEDULES.save(
      deps.storage,
      deps.api.addr(&beneficiary)?.as_slice(),
      &schedule,
    );
...
pub fn release(
  ...
  if current_time < schedule.vesting_start {
    return Err(StdError::genric_err("lock period"));
  }
  ...
  if payout.is_zero() {
    return Err(StdError::genric_err("nothing to release"))
  }
